...
public class SportEventWinner
{
    ...
    private async Task InvokeAgent(AgentTask agentTask, CancellationToken cancellationToken)
    {
        ...
        string finalResponse = "The Munich Flying Dolphins won in Munich.";
        await _taskManager!.ReturnArtifactAsync(agentTask.Id, new Artifact()
        {
            Parts = [
                new TextPart(){
        ...
        Message message = new Message()
        {
            ...
            Parts = [
                new TextPart(){
                    Text = $"Agent input: \t{inputText}"
                },
                new TextPart(){
                    Text = $"Agent response: \t{finalResponse}"
                }
            ]   
        };
    ...
    private Task<AgentCard> AgentCardQueryHandler(string agentUrl, CancellationToken cancellationToken)
    {
        ...
        {
            Name = "Sport Event Winner Task",
            Description = "Agent provides sport event winner and location as task",
            Url = agentUrl,
            Version = "1.0.0",
            DefaultInputModes = ["Sport event name & sport event year"],
            DefaultOutputModes = ["Sport event winner & location"],
